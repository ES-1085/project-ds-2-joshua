...
grid.arrange(p1, p2, p3, p4, p5, p6, p7, p8, p9, top = textGrob("Relationships between P. cinereus abundance and predictors"))
```
#### Step-wise AIC
...
```{r}
library(AICcmodavg)
models <- list(hurdle_nb1, hurdle_nb2, hurdle_nb3)
mod.names <- c('Env model full', 'Env model reduced', 'Env model ft interaction')
aictab(cand.set = models, modnames = mod.names)
```

```{r}
nb1 <- glm.nb(n_ind ~ canopy.cover, data = df)
summary(nb1)
pred.plot(nb1, xpred = "canopy.cover") +
  labs(x = "Volume CWD (m2/ha)",
       y = "P. cinereus abundance per cover board")
```
...
```
Mean P. cinereus  is significantly higher in hemlock-dominated stands than in mixed-deciduous stands (Wilcoxon rank sum test, W = 1676, P = <0.001).
