...
ggplot(mass_towns) +
  geom_sf() +
  geom_count(data = coords_filtered, (aes(x = long_dd, y = lat_dd)), alpha = 0.25) +
  xlim(-72.5, -72) +
  ylim(42.4, 42.7)
```
...
df_pc <- df_pc %>%
  dplyr::select(plot, site, ft, asn, aco, ba, st.ha, ts, canopy.cover, cwd.no, cwd.con, vcwd, vcwd.con, avg.dc, tot.snags, avg.snag.dbh, soil.ph, date, airt, rh, soilt, n_ind, pcn, pc.no, p, svl, tl, wt, sex)
```
...
grid.arrange(box1, box2, box3, ncol = 3, top = textGrob("P. cinereus morphometrics by forest type"))
```
...
pc_means %>%
  ...
  summarize(count = n()) %>%
  ggplot() +
  geom_col(aes(x = sex, y = count, fill = as.factor(cluster))) +
  coord_flip() +
  labs(fill = "Cluster")
```
