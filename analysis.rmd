...
## Data Tidying
Redoing some initial analysis here by using `uncount` function to duplicate rows for which there are multiple salamanders observed; currently each observation is a count associated with a cover board, but I want each observation to be an individual salamander.
...
df_pc$aco = as.character(df_pc$aco)
df_pc$asn = as.character(df_pc$asn)

#select useful columns
df_pc <- df_pc %>%
  select(plot, site, ft, asn, aco, ba, st.ha, ts, canopy.cover, cwd.no, cwd.con, vcwd, vcwd.con, avg.dc, tot.snags, avg.snag.dbh, soil.ph, date, airt, rh, soilt, n_ind, pcn, pc.no, p, svl, tl, wt, sex)
```

It is worth noting that because each row in the `aco` dataframe represents a sampling event at a specific cover board rather than an individual P. cinereus, there are multiple unique identifiers lumped into the `pcn` column but these are individual values in the `pc.no` column from the `pc` dataframe (which does treat every row as an individual P. cinereus).  These rows do not line up as a result, but they are matched to the appropriate 
```{r}
#df_pc %>%
#  mutate(presence = case_when(pca >= 1 ~ 1, pca == 0 ~ pca))
```
Need to change `n_ind` -- it looks like it's not in line with other rows in the dataset.
...
#### Step-wise AIC comparisons
```{r}
fit = lm(n_ind ~ ts + ba + avg.dc + vcwd.con + soil.ph + rh, data = df_pc)
...
summary(nb_hurdle1)
```
#### Hurdle NB with forest type interaction
```{r}
nb_hurdle2 <- hurdle(n_ind ~ ft * ts * ba + st.ha + canopy.cover + cwd.con * vcwd.con + avg.dc + soil.ph + airt * soilt + rh, data = df_pc, dist = "negbin")
summary(nb_hurdle2)
```
...
```{r}
models <- list(nb_hurdle1, nb_hurdle2)
mod.names <- c('Environmental model Hurdle NB', 'Env model w/ forest type')
aictab(cand.set = models, modnames = mod.names)
```
